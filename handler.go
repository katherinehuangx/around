...
var mySigningKey = []byte("***")   // string "***" should be replaced by real private key
...
func searchHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil {
        http.Error(w, "Failed to read post from Elasticsearch", http.StatusInternalServerError)   // error code 500
        fmt.Printf("Failed to read post from Elasticsearch %v.\n", err)
        return
    }

    js, err := json.Marshal(posts)    // opposite to decode
    if err != nil {
...
func signinHandler(w http.ResponseWriter, r *http.Request) {
    ...
    w.Header().Set("Content-Type", "text/plain")   // return token which is a string.
    w.Header().Set("Access-Control-Allow-Origin", "*")
    ...
    token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
        ...
        "exp":      time.Now().Add(time.Hour * 24).Unix(),   // expire in 24 hrs
    })
